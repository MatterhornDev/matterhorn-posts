...
- [1 Set Up Guide](#1-set-up-guide)
  ...
  - [1.3 Adding Standard Style Formatter](#13-adding-standard-style-formatter)
    - [1.3.1 Installing Standard](#131-installing-standard)
    - [1.3.2 Evaluating new errors](#132-evaluating-new-errors)
    - [1.3.3 Configuring Standard specific rules](#133-configuring-standard-specific-rules)
  - [1.4 Adding Prettier Style Formatter](#14-adding-prettier-style-formatter)
    - [1.4.1 Installing Prettier](#141-installing-prettier)
    - [1.4.2 Configuring Prettier specific rules](#142-configuring-prettier-specific-rules)
...
#### 1.3.3 Configuring Standard specific rules
...
#### 1.4.1 Installing Prettier

Just like Standard, Prettier is very easy to get up and running with the existing ESLint configuration. Start by running the following command:
...
#### 1.4.2 Configuring Prettier specific rules

As mentioned previously, the purpose of using a formatter such as prettier is so that there is no configuration needed. Nevertheless, understanding how to modify rules is important to comply to an organizations formatting practices. Lets use the classic `semicolon` rule as an example.

To modify Prettier rules you must create either a new `.prettierrc.json` file or add a `"prettier"` section to the `package.json`. In order to eliminate additional files, I prefer to add it directly to the package.json.
